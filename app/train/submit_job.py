...
def create_gcs_training_data(project_id: str, bucket_name: str, local_csv_path: Optional[str] = None):
    """创建或上传训练数据到GCS"""
    print(f"📊 准备训练数据...")
    ...
    try:
        ...
        if local_csv_path and os.path.exists(local_csv_path):
            ...
            print(f"📤 上传本地数据: {local_csv_path} -> gs://{bucket_name}/{blob_name}")
            ...
            # 检查GCS中是否已有数据文件
            data_blob = bucket.blob("data/btc_1m.csv")
            if data_blob.exists():
                print(f"✅ 使用现有GCS数据: gs://{bucket_name}/data/btc_1m.csv")
                return f"gs://{bucket_name}/data/btc_1m.csv"
            else:
                print(f"⚠️ 未找到训练数据文件，训练将使用模拟数据")
                return None
...
def submit_vertex_training_job(
    ...
    try:
        ...
        job = aiplatform.CustomTrainingJob(
            ...
            container_uri=f"gcr.io/cloud-aiplatform/training/pytorch-gpu.1-13.py{python_version.replace('.', '')[:2]}:latest",
            requirements=[
        ...
        print(f"   - 在Google Cloud Console查看: https://console.cloud.google.com/vertex-ai/training/custom-jobs/{job_resource_name}?project={project_id}")
...
def main():
    ...
    parser.add_argument("--project_id", type=str, required=True,
                       help="GCP项目ID")
    parser.add_argument("--region", type=str, default="asia-southeast1",
                       help="Vertex AI区域")
    ...
    parser.add_argument("--list_jobs", action="store_true",
                       help="列出最近的训练任务")
    parser.add_argument("--check_job", type=str,
                       help="检查指定任务状态 (提供job resource name)")
    ...
    try:
        ...
        print("💡 使用以下命令检查状态:")
        print(f"   python {__file__} --project_id {args.project_id} --region {args.region} --bucket {args.bucket} --check_job {job_resource_name}")
        print("=" * 60)
