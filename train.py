...
def resolve_data_path(path: str) -> str:
    """将本地路径或 gs:// 路径解析为【本地】可读文件；失败直接抛错。"""
    if not path:
        raise ValueError("必须提供 --data_csv 且不可为空。")
    ...
    if path.startswith("gs://"):
        ...
        if not blob.exists():
            raise FileNotFoundError(f"GCS 文件不存在: {path}")
        fd, tmp_path = tempfile.mkstemp(prefix="btc_", suffix=".csv")
        os.close(fd)
        blob.download_to_filename(tmp_path)
        print(f"✅ 已从 GCS 下载到本地: {tmp_path}")
        return tmp_path
    else:
...
def train_ppo_model(
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=10000, save_path="./checkpoints/", name_prefix="ppo_btc"
    )
...
def evaluate_model(model: PPO, env: BTCTradingEnv, n_episodes: int = 10) -> dict:
    ...
    for episode in range(n_episodes):
        ...
        print(
            f"   Episode {episode + 1}: Return={episode_return:.4f}, "
...
def main():
    ...
    parser.add_argument(
        "--data_csv", type=str, default="btc_data.csv", help="BTC历史数据CSV文件路径或GCS路径"
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default="gs://your-bucket/models/ppo/",
        help="模型输出GCS路径",
    )
    parser.add_argument("--timesteps", type=int, default=100000, help="总训练步数")
    parser.add_argument("--lookback", type=int, default=60, help="历史数据回望长度")
    parser.add_argument("--lr", type=float, default=3e-4, help="学习率")
    parser.add_argument("--eval", action="store_true", help="训练后进行模型评估")
    parser.add_argument(
        "--check_only", action="store_true", help="仅检查环境，不进行训练"
    )
    ...
    # 解析数据路径(支持GCS)
    try:
        local_data_path = resolve_data_path(args.data_csv)
        print(f"✅ 数据路径解析成功: {local_data_path}")
    except Exception as e:
        print(f"❌ 数据路径解析失败: {e}")
        return

    # 检查环境
    if not check_btc_env(local_data_path):
        print("❌ 环境检查失败，训练终止")
        return
    ...
    try:
        # 创建训练环境
        env = create_training_env(local_data_path, args.lookback)

        # 训练模型
        model = train_ppo_model(
            env=env, total_timesteps=args.timesteps, learning_rate=args.lr, verbose=1
        )
        ...
        # 清理临时文件
        if 'local_data_path' in locals() and local_data_path.startswith('/tmp/'):
            try:
                os.remove(local_data_path)
                print(f"🗑️ 已清理临时文件: {local_data_path}")
            except Exception:
                pass
